...
(async () => {
  ...
  // Capture runtime errors and unhandled rejections from the page
  page.on('pageerror', err => console.log('PAGE_ERROR', err && (err.stack || err.message)));
  page.on('error', err => console.log('PAGE_ERROR_FATAL', err && (err.stack || err.message)));

  // Helper to dump a compact snapshot of window.app and a short body HTML preview
  async function dumpAppState(tag) {
    try {
      const snap = await page.evaluate(() => {
        try {
          ...
          const safe = app ? {
            ...
            currentUser: (app.currentUser && (app.currentUser.phone || app.currentUser.id || app.currentUser.name)) || null,
          ...
          let body = document.body ? (document.body.innerText || '').slice(0, 1200) : '';
          const hasPickup = !!document.querySelector('#pickupForm');
          const ch = document.getElementById('customerHome');
          const chPreview = ch ? ((ch.innerText || '').slice(0,800)) : null;
          const readyFlag = !!(window.__CUSTOMER_HOME_READY);
          return { safe, bodyPreview: body, hasPickupForm: hasPickup, customerHomePreview: chPreview, customerHomeReady: readyFlag };
        } catch (e) { return { error: String(e) }; }
      });
      console.log('DUMP_APP_STATE', tag, JSON.stringify(snap));
    } catch (e) { console.log('DUMP_APP_STATE_FAILED', tag, e && e.stack); }
  }
  ...
  try {
  const startUrl = process.env.TEST_BASE_URL || 'http://localhost:3001/home.html';
  console.log('ASSIGN_FLOW_TEST starting at', startUrl);
  await page.goto(startUrl, { waitUntil: 'networkidle2', timeout: 60000 });
  // extra settle time for slower environments
  ...
  await page.waitForSelector('#loginModal', { timeout: 20000 });
  await dumpAppState('after_admin_showLoginModal');
  ...
  await dumpAppState('after_customer_showLoginModal');
  ...
  await click('#modalLoginSubmit');
  await page.waitForSelector('#customerHome', { timeout: 20000 });
    await click('[data-action="requestPickup"]');
  // Wait for customerHome to be fully initialized (fragment injected or fallback form present)
  try {
    await page.evaluate(() => {
      if (window.__CUSTOMER_HOME_READY) return true;
      return new Promise((res) => {
        function onReady() { document.removeEventListener('customerHomeReady', onReady); res(true); }
        document.addEventListener('customerHomeReady', onReady);
        // safety timeout inside page in case event never fires
        setTimeout(() => { try { document.removeEventListener('customerHomeReady', onReady); res(false); } catch(e){} }, 5000);
      });
    });
  } catch(e) {}
  // small settle and dump state so we can see why pickup form might not appear
  await new Promise(r => setTimeout(r, 600));
  await dumpAppState('before_wait_pickupForm');
  await page.waitForSelector('#pickupForm', { timeout: 30000 });
