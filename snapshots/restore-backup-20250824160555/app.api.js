...
(function() {
  ...
  attempts.push(abs('http://localhost:3000', url));
    ...
    // Expose a quick refresher for requests based on current auth token/phone
    window.__refreshRequests = async function() {
      try {
        const normPhone = (app.currentUser && app.currentUser.phone) ? (window.__normalizePhone10 ? window.__normalizePhone10(app.currentUser.phone) : app.currentUser.phone) : '';
        const phoneQuery = normPhone ? ('?phone=' + encodeURIComponent(normPhone)) : '';
        const reqs = await fetchJson('/api/requests' + phoneQuery);
        if (Array.isArray(reqs)) {
          app.requests = reqs.map(r => {
      ...
      } catch(e) { console.warn('__refreshRequests failed', e && e.message); }
    };
  ...
  // No forced default view; let the user choose. Only re-init current role if set.
    } catch (e) {}
